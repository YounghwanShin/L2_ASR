...
class MpdStats(MetricStats):
    ...
    def append(
        ...
        assert len(canonical) == len(perceived)
...
def main(args):
    ...
    per_file = open(args.per_file, "w") if args.per_file else None
    mpd_file = open(args.mpd_file, "w") if args.mpd_file else sys.stdout
    stats = mpd_eval_on_dataset(json_data, mpd_file, per_file)
    return stats
