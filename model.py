...
class PhonemeRecognitionModel(nn.Module):
    ...
    def __init__(self, 
        ...
        # 두 번째 wav2vec: 학습 가능한 파라미터 (동일하게 XLSR-53)
        ...
        # 특징 융합 (이제 차원이 동일함)
        config = Wav2Vec2Config.from_pretrained(pretrained_model_name)
        feature_dim = config.hidden_size  # 1024
        
        self.feature_fusion = FeatureFusion(
            input_dim1=feature_dim,
            input_dim2=feature_dim,
    ...
    def forward(self, x, attention_mask=None):
        ...
        frozen_features = self.frozen_wav2vec(x, attention_mask)  # [B, T, 1024]
        learnable_features = self.learnable_wav2vec(x, attention_mask)  # [B, T, 1024]
        
        # 특징 융합 (단순 concatenation)
        fused_features = self.feature_fusion(frozen_features, learnable_features)
