"""Loss functions for pronunciation assessment.

This module implements Focal CTC Loss and unified multitask loss combining
canonical phoneme, perceived phoneme, and error detection objectives.
"""
...
class FocalCTCLoss(nn.Module):
  """Focal Loss applied to CTC for handling class imbalance."""
  ...
  def forward(
    ...
    Args:
      ...
      input_lengths: Input lengths of shape [N].
      target_lengths: Target lengths of shape [N].
...
class UnifiedLoss(nn.Module):
  """Unified multitask loss for pronunciation assessment.
  
  Combines canonical phoneme recognition, perceived phoneme recognition,
  and error detection losses with configurable weights.
  """
  ...
  def forward(
    ...
    """Computes unified multitask loss.
    
    Args:
      outputs: Model outputs dictionary with logits.
    ...
    """
    total_loss = torch.tensor(0.0, device=outputs[list(outputs.keys())[0]].device)
    loss_dict = {}
