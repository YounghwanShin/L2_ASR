"""Model evaluator for pronunciation assessment.

This module implements evaluation logic for phoneme recognition and
error detection with per-speaker analysis.
"""
...
class ModelEvaluator:
  """Evaluator for pronunciation assessment tasks."""
  ...
  def _clean_labels(
    ...
    """Removes padding from label sequences.
    
    Args:
      labels: Padded label tensor of shape [batch_size, max_len].
      lengths: Label lengths of shape [batch_size].
    
    Returns:
      List of label sequences without padding.
    """
    return [
        labels[i][:length].cpu().numpy().tolist()
  ...
  def _decode_predictions(
    ...
    """Decodes CTC predictions from logits.
    
    Args:
      logits: Model logits of shape [batch_size, seq_len, vocab_size].
      audio_lengths: Audio lengths of shape [batch_size].
      model: Model instance for length computation.
  ...
  def evaluate_phoneme_recognition(
    ...
    Returns:
      Dictionary with evaluation metrics and per-speaker results.
    """
    model.eval()
    ...
    with torch.no_grad():
      for batch_data in tqdm(dataloader, desc=f'{target_type.capitalize()} Eval'):
        ...
        predictions = self._decode_predictions(
            outputs[logits_key], audio_lengths, model
        )
        targets = self._clean_labels(
            batch_data[labels_key], batch_data[lengths_key]
        )
        ...
        # Collect canonical and perceived if available
        if 'canonical_labels' in batch_data and 'perceived_labels' in batch_data:
          canonical = self._clean_labels(
              batch_data['canonical_labels'], batch_data['canonical_lengths']
          )
          perceived = self._clean_labels(
              batch_data['perceived_labels'], batch_data['perceived_lengths']
          )
  ...
  def evaluate_error_detection(
    ...
    with torch.no_grad():
      for batch_data in tqdm(dataloader, desc='Error Detection Eval'):
        ...
        # Decode predictions
        predictions = self._decode_predictions(
            outputs['error_logits'], audio_lengths, model
        )
        targets = self._clean_labels(
            batch_data['error_labels'], batch_data['error_lengths']
        )
    ...
    # Calculate overall metrics
    results = calculate_error_metrics(
        all_predictions, all_targets, all_ids, error_type_names
    )
