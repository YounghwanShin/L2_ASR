...
class ModelEvaluator:
  """Evaluator for pronunciation assessment model with multitask support."""
  ...
  def _clean_labels(
    ...
    Args:
      ...
      label_lengths: Label sequence lengths of shape [batch_size].
  ...
  def evaluate_phoneme_recognition(
    ...
    Args:
      ...
      target_type: Target type ('canonical' or 'perceived').
      
    Returns:
      Dictionary containing evaluation metrics including per-speaker results.
    """
    model.eval()
    ...
    with torch.no_grad():
      for batch_data in tqdm(
          dataloader, 
          desc=f'{target_type.capitalize()} Phoneme Eval'
      ):
        ...
        targets = self._clean_labels(
            batch_data[labels_key], 
            batch_data[lengths_key]
        )
        ...
        # Collect canonical and perceived for mispronunciation detection
        if ('canonical_labels' in batch_data and 
            'perceived_labels' in batch_data):
          canonical = self._clean_labels(
              batch_data['canonical_labels'],
              batch_data['canonical_lengths']
          )
          perceived = self._clean_labels(
              batch_data['perceived_labels'],
  ...
  def evaluate_error_detection(
    ...
    Returns:
      Dictionary containing error detection metrics including per-speaker results.
    """
    model.eval()
    all_predictions, all_targets = [], []
    all_ids, all_speaker_ids = [], []
    ...
    with torch.no_grad():
      for batch_data in tqdm(dataloader, desc='Error Detection Eval'):
        ...
        input_lengths = torch.clamp(
            input_lengths, 
            min=1, 
            max=error_logits.size(1)
        )

        log_probs = torch.log_softmax(error_logits, dim=-1)
        predictions = greedy_ctc_decode(log_probs, input_lengths)
        targets = self._clean_labels(
            batch_data['error_labels'],
