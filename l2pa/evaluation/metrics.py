...
This module implements evaluation metrics including edit distance,
...
def compute_edit_distance_with_details(
    reference: List, 
    hypothesis: List
) -> Tuple[int, int, int, int, int]:
  """Computes Levenshtein edit distance with error type breakdown.
  
  Uses dynamic programming to compute edit distance and backtracks
  to count substitutions, deletions, and insertions.
...
def calculate_sequence_error_rate(
    ids: List, 
    references: List[List], 
    hypotheses: List[List]
) -> List[Dict]:
  """Calculates sequence error rate with detailed breakdown.
  ...
  Returns:
    List of dictionaries containing error metrics for each sample.
  """
  details = []
  ...
  for sample_id, reference, hypothesis in zip(ids, references, hypotheses):
    ...
    (total_errors, substitutions, deletions, 
     insertions, num_ref) = compute_edit_distance_with_details(
...
def convert_ids_to_phonemes(
  ...
  """Converts phoneme ID sequences to phoneme strings.
  ...
  """
  return [
      [id_to_phoneme.get(str(token_id), f"UNK_{token_id}") 
       for token_id in seq] 
...
def calculate_mispronunciation_metrics(
  ...
  errors by comparing predicted to canonical and perceived phonemes.
  ...
  for pred, canonical, perceived in zip(
      pred_phonemes, canonical_phonemes, perceived_phonemes
  ):
    ...
    for canonical_phone, perceived_phone, predicted_phone in zip(
        canonical_padded, perceived_padded, pred_padded
    ):
      if '<pad>' in [canonical_phone, perceived_phone, predicted_phone]:
        continue
  ...
  recall = (total_true_rejection / (total_true_rejection + total_false_acceptance) 
            if (total_true_rejection + total_false_acceptance) > 0 else 0.0)
  precision = (total_true_rejection / (total_true_rejection + total_false_rejection) 
               if (total_true_rejection + total_false_rejection) > 0 else 0.0)
...
def calculate_error_metrics(
  ...
  correct_sequences = sum(
      1 for detail in error_rate_details 
      if detail['error_rate'] == 0.0
  )

  total_tokens = sum(
      detail['num_reference_tokens'] 
      for detail in error_rate_details
  )
  total_errors = sum(
      detail['insertions'] + detail['deletions'] + detail['substitutions'] 
      for detail in error_rate_details
  )
  ...
  if len(flat_predictions) > 0 and len(flat_targets) > 0:
    try:
      ...
      evaluated_error_types = {
          k: v for k, v in error_type_names.items() if k != 0
      }
