...
def compute_edit_distance(reference: List, hypothesis: List) -> Tuple[int, int, int, int]:
...
def calculate_phoneme_metrics(
  ...
  target_phonemes = remove_silence(convert_ids_to_phonemes(targets, id_to_phoneme))
  ...
  misp_metrics = {'precision': 0.0, 'recall': 0.0, 'f1_score': 0.0,
                  'true_acceptance': 0, 'false_rejection': 0,
                  'false_acceptance': 0, 'true_rejection': 0}
...
def _calculate_mispronunciation_metrics(predictions, canonical, perceived, id_to_phoneme):
  ...
  return {'precision': precision, 'recall': recall, 'f1_score': f1,
          'true_acceptance': ta, 'false_rejection': fr,
          'false_acceptance': fa, 'true_rejection': tr}


def calculate_error_metrics(predictions, targets, ids, error_type_names):
